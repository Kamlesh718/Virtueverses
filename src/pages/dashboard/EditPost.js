...
function EditPost() {
  ...
  const post = blogsData.find((blog) => blog.id === parseInt(userId, 10));
  const [title, setTitle] = useState(post.title);
  const [description, setDescription] = useState(post.description);
  const [selectedOption, setSelectedOption] = useState(post.id);
      ...
      >
        ...
        <div className="flex flex-col">
          ...
          <textarea
            ...
            className="p-2 rounded-md border border-violet-700 focus:outline-none focus:border-violet-900 text-violet-950 h-[8vh]"
          />
        </div>
        <div className="flex flex-col">
